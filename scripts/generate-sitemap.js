...
function getRecipeData(filename) {
  try {
    ...
    if (imagesMatch) {
      ...
      const largeImageMatch = imagesContent.match(/large:\s*['"`]([^'"`]+)['"`]/);
      if (largeImageMatch) {
    ...
    console.warn(`Warning: Could not parse recipe file ${filename}.ts:`, error.message);
    return {
...
function main() {
  try {
    ...
    console.log(`📄 Sitemap contains ${(sitemapContent.match(/<url>/g) || []).length} URLs`);
