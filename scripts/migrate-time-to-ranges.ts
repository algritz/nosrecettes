...
async function migrateRecipeFile(filePath: string, options: MigrationOptions): Promise<boolean> {
  try {
    ...
    transformedContent = transformedContent.replace(
      /(\s+prepTime:\s*)(\d+)(,?)/g,
      (match, prefix, value, comma) => `${prefix}{ min: ${value}, max: ${value} }${comma}`
    );
...
async function migrateAllRecipes(options: MigrationOptions): Promise<MigrationResult> {
  ...
  console.log(`Mode: ${options.dryRun ? 'DRY RUN (no files will be modified)' : 'LIVE'}`);
...
const backupDir = path.join(projectRoot, 'scripts', 'backups', `migration-${timestamp}`);
