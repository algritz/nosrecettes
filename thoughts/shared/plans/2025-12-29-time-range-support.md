...
async function migrateRecipeFile(filePath: string, options: MigrationOptions): Promise<boolean> {
  // 1. Read file content
  // 2. Parse to AST
...
async function createBackup(recipesDir: string, backupDir: string): Promise<void> {
  // Copy entire recipes directory to backup location
}

async function migrateAllRecipes(options: MigrationOptions): Promise<MigrationResult> {
  // 1. Create backup
...
console.log(`Migration complete: ${result.processed} processed, ${result.skipped} skipped, ${result.errors.length} errors`);
```
...
const [marinatingTime, setMarinatingTime] = useState<TimeRange>({ min: 0, max: 0 });
```
...
async function handleSubmit(e: FormEvent) {
  ...
  const marinatingTimeNum = marinatingTime ? parseInt(marinatingTime, 10) : undefined;
...
async function handleSubmit(e: FormEvent) {
  ...
  const recipeData = {
    ...
    marinatingTime: (marinatingTime.min > 0 || marinatingTime.max > 0) ? marinatingTime : undefined,
    // ... rest of fields ...
  };
...
useEffect(() => {
  if (recipe) {
    ...
    setMarinatingTime(recipe.marinatingTime ? recipe.marinatingTime.toString() : '');
  }
}, [recipe]);
...
function parseCSVRowToRecipe(row: any): ParsedCSVRecipe {
  return {
    ...
    marinatingTime: row.marinatingTime ? parseTimeField(row.marinatingTime) : undefined,
...
function parseTimeField(value: string | number): TimeRange {
  ...
  if (strValue.includes('-')) {
    ...
    // Validate range
    if (max < min) {
      console.warn(`Invalid time range in CSV: ${strValue} (max < min). Using min value for both.`);
      return { min, max: min };
    }
...
export function parseCSVTimeField(value: string | number): TimeRange {
  ...
  if (strValue.includes('-')) {
    ...
    if (parts.length !== 2) {
      console.warn(`Invalid range format in CSV: "${strValue}". Expected "min-max". Using 0.`);
      return { min: 0, max: 0 };
    }
    ...
    if (isNaN(min) || isNaN(max)) {
      console.warn(`Invalid numeric values in CSV range: "${strValue}". Using 0.`);
      return { min: 0, max: 0 };
    }

    // Validate min <= max
    if (max < min) {
      console.warn(`Invalid range in CSV: "${strValue}" (max < min). Swapping values.`);
...
export function parseCSVToRecipes(csvContent: string): ParsedCSVRecipe[] {
  ...
  return rows.map(row => ({
    ...
    marinatingTime: row.marinatingTime ? parseCSVTimeField(row.marinatingTime) : undefined,
...
```typescript
const marinatingTimeField = data.marinatingTime && data.marinatingTime.min > 0
  ? `\n  marinatingTime: { min: ${data.marinatingTime.min}, max: ${data.marinatingTime.max} },`
  : '';
```
...
```typescript
const marinatingInfo = recipeData.marinatingTime && getMaxTime(recipeData.marinatingTime) > 0
