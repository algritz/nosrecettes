...
interface GitHubSetupProps {
  onConfigSaved: (config: { owner: string; repo: string; token: string }) => void;
}
  ...
  const handleTestConnection = async () => {
    try {
      ...
      if (response.ok) {
        ...
        setIsValid(false);
        alert('Impossible de se connecter au repository. Vérifiez vos informations.');
      }
    } catch (error) {
      setIsValid(false);
  ...
  return (
    <Card className="max-w-2xl mx-auto">
      ...
      <CardContent className="space-y-4">
        <Alert>
          <AlertDescription>
            Pour utiliser la création automatique de pull requests, vous devez configurer l'accès à votre repository GitHub.
          </AlertDescription>
        </Alert>

        <div className="space-y-4">
          <div>
            <label className="block text-sm font-medium mb-2">Propriétaire du repository</label>
            <Input
              value={config.owner}
          ...
          <div>
            <label className="block text-sm font-medium mb-2">Nom du repository</label>
            <Input
              value={config.repo}
          ...
          <div>
            ...
            <p className="text-xs text-muted-foreground mt-1">
              Le token doit avoir les permissions "repo" pour créer des branches et pull requests.
            </p>
          </div>
        </div>

        <div className="flex gap-2">
          <Button onClick={handleTestConnection} disabled={!config.owner || !config.repo || !config.token}>
            Tester la connexion
          </Button>
          {isValid && (
