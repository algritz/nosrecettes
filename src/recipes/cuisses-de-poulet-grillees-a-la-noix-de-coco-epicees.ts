import type { Recipe } from '@/types/recipe'

export const cuissesDePouletGrilleesALaNoixDeCocoEpicees: Recipe = {
  id: 'cuisses-de-poulet-grillees-a-la-noix-de-coco-epicees',
  title: 'Cuisses de poulet grillées à la noix de coco épicées',
  description:
    'Une recette de cuisses de poulet épicées et parfumées à la noix de coco, grillées au barbecue et servies avec de la coriandre et du citron vert. Aussi connu sous le nom de "Emery\'s gone in 60 seconds chicken"',
  categories: ['Vollaille', 'Plats principaux'],
  prepTime: { min: 20, max: 20 },
  cookTime: { min: 15, max: 15 },
  marinatingTime: { min: 480, max: 480 },
  servings: 4,
  difficulty: 'Facile',
  ingredients: [
    '1 morceau de gingembre frais de 5 cm',
    '5 gousses d’ail',
    '¾ tasse de lait de coco',
    '¼ tasse de sambal oelek',
    '¼ tasse de jus de lime frais',
    '2 c. à soupe de cassonade',
    '1 c. à soupe de sel casher',
    '2 c. à soupe d’huile végétale',
    '2 lb. de haut de cuisse de poulet',
    '½ tasse de feuilles de coriandre hachée',
    'Quartiers de lime (pour servir)',
  ],
  instructions: [
    'Râper finement le gingembre et l’ail dans un bol moyen.',
    'Ajouter le lait de coco, le sambal Oelek, le jus de lime, la cassonade, le sel et 2 c. à soupe d’huile, puis fouetter pour combiner.',
    'Ajouter le poulet et bien brasser pour enrober. Laisser reposer au moins 4 à 8 heures.',
    'Retirer le poulet de la marinade, en laissant l’excès s’égoutter dans le bol, et transférer sur une plaque à pâtisserie bordée.',
    'Verser la marinade dans une petite casserole. Porter à ébullition la marinade, puis réduire le feu à doux et laisser mijoter, en remuant de temps en temps, jusqu’à ce qu’elle soit légèrement réduite et suffisamment épaisse pour recouvrir le dos d’une cuillère, 2 à 3 minutes.',
    'Préparer le barbecue à feu moyen. Faire griller le poulet sur un barbecue bien huilé en le retournant une fois et en badigeonnant de temps en temps avec de la marinade, jusqu’à ce que vous voyiez de bonnes marques de grill et que le poulet soit cuit, 8 à 10 minutes.',
    'Badigeonner avec le reste de la marinade.',
    'Garnir de coriandre et servir avec des quartiers de citron vert à côté.',
  ],
  tags: ['barbecue', 'épicé', 'coco'],
  images: [
    {
      small:
        'https://res.cloudinary.com/nosrecettes/image/upload/w_400,h_300,c_fill,q_auto,f_auto/recipes/emery_gone_in_60_sec_chicken',
      medium:
        'https://res.cloudinary.com/nosrecettes/image/upload/w_800,h_600,c_fill,q_auto,f_auto/recipes/emery_gone_in_60_sec_chicken',
      large:
        'https://res.cloudinary.com/nosrecettes/image/upload/w_1200,h_900,c_fill,q_auto,f_auto/recipes/emery_gone_in_60_sec_chicken',
    },
  ],
  accompaniment: 'Riz basmati',
  source: 'David Cloutier',
  slug: 'cuisses-de-poulet-grillees-a-la-noix-de-coco-epicees',
}
