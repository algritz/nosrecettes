...
export const clearScheduledCleanups = (publicIds?: string[]): void => {
  try {
    ...
    const filteredRecords = existingRecords.filter(
      (record) => !publicIds.includes(record.publicId),
    )
...
export const generateCleanupInstructions = (): string => {
  ...
  const groupedByReason = records.reduce(
    (acc, record) => {
  ...
  instructions +=
    'The following images will be automatically cleaned up by GitHub Action after this PR is merged:\n\n'
  ...
  instructions +=
    '- ✅ GitHub Action will run automatically when this PR is merged\n'
...
export const extractPublicIdFromUrl = (url: string): string | null => {
  try {
    ...
    const multiTransformMatch = url.match(
      /\/image\/upload\/.*?\/([^/]+)\.(jpg|jpeg|png|gif|webp)$/i,
    )
