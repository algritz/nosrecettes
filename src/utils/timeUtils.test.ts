...
describe('TimeRange Utilities', () => {
  ...
  describe('validateTimeRange', () => {
    ...
    it('throws for max < min', () => {
      expect(() => validateTimeRange({ min: 30, max: 20 })).toThrow('min (30) cannot be greater than max (20)');
    });

    it('throws for negative min', () => {
      expect(() => validateTimeRange({ min: -5, max: 10 })).toThrow('times must be non-negative');
    });

    it('throws for negative max', () => {
      expect(() => validateTimeRange({ min: 10, max: -5 })).toThrow('times must be non-negative');
    });
    ...
    it('throws for invalid object (non-TimeRange)', () => {
      expect(() => validateTimeRange({ min: '10', max: 20 } as any)).toThrow('must be an object with numeric min and max properties');
    });
  });
  ...
  describe('Factory functions', () => {
    describe('createExactTime', () => {
      ...
      it('throws for Infinity', () => {
        expect(() => createExactTime(Infinity)).toThrow('Invalid time value: Infinity');
      });
    ...
    describe('createTimeRange', () => {
      ...
      it('throws when max < min', () => {
        expect(() => createTimeRange(30, 20)).toThrow('min (30) cannot be greater than max (20)');
      });

      it('throws for negative min', () => {
        expect(() => createTimeRange(-5, 10)).toThrow('times must be non-negative');
      });

      it('throws for negative max', () => {
        expect(() => createTimeRange(10, -5)).toThrow('times must be non-negative');
      });
    });
  });
});
